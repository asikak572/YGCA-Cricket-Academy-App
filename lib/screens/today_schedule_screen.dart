...
class _TodayScheduleScreenState extends State<TodayScheduleScreen> {
  ...
  List<_TodayScheduleItem> todayItems = [];
  ...
  Future<void> _loadCoachAssignedBatches(Set<String> batches) async {
    try {
      ...
      // Keeps screen safe if collection/index is not available yet.
    }
  }
  ...
  Future<void> _loadTodaySchedule() async {
    ...
    try {
      ...
      final loadedItems = <_TodayScheduleItem>[];

      if (!_roleNeedsBatchButEmpty(loadedRole, batches)) {
        loadedItems.addAll(
          await _loadTrainingItems(
      ...
      loadedItems.sort((a, b) => a.time.compareTo(b.time));
  ...
  Future<List<_TodayScheduleItem>> _loadTrainingItems({
    required String loadedRole,
    required Set<String> batches,
  }) async {
    final result = <_TodayScheduleItem>[];
    ...
    try {
      ...
      // Keep screen stable if this collection is not available.
    }

    return result;
  }

  Future<List<_TodayScheduleItem>> _loadMatchItems({
    required String loadedRole,
    required Set<String> batches,
  }) async {
    final result = <_TodayScheduleItem>[];
  ...
  Future<List<_TodayScheduleItem>> _loadCancelledItems({
    required String loadedRole,
    required Set<String> batches,
  }) async {
    final result = <_TodayScheduleItem>[];
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        if (!_isTodayDate(dateValue)) continue;
  ...
  bool _isTodayTraining(Map<String, dynamic> data) {
    final dateValue = data['date'];

    if (dateValue != null && _text(dateValue).isNotEmpty) {
      return _isTodayDate(dateValue);
    }

    final day = _text(data['day']).toLowerCase();

    return day == _todayDayName.toLowerCase();
  }

  bool _isTodayDate(dynamic value) {
  ...
  Widget _summaryRow(bool isDark) {
    final trainingCount =
        todayItems.where((item) => item.category == "Training").length;
    final matchCount = todayItems.where((item) => item.category == "Match").length;
    final cancelledCount =
        todayItems.where((item) => item.category == "Cancelled").length;
  ...
  Widget _scheduleTile({
    required bool isDark,
    required _TodayScheduleItem item,
  }) {
    return Container(
