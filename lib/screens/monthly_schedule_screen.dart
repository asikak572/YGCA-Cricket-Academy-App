...
class _MonthlyScheduleScreenState extends State<MonthlyScheduleScreen> {
  ...
  List<_MonthlyScheduleItem> monthlyItems = [];
  ...
  DateTime _endOfMonth() {
    return DateTime(_now.year, _now.month + 1, 0, 23, 59, 59);
  }
  ...
  Future<void> _loadMonthlySchedule() async {
    ...
    try {
      ...
      final loadedItems = <_MonthlyScheduleItem>[];
  ...
  Future<List<_MonthlyScheduleItem>> _loadTrainingItems({
    required String loadedRole,
    required Set<String> batches,
  }) async {
    final result = <_MonthlyScheduleItem>[];
  ...
  Future<List<_MonthlyScheduleItem>> _loadMatchItems({
    required String loadedRole,
    required Set<String> batches,
  }) async {
    final result = <_MonthlyScheduleItem>[];
  ...
  Future<List<_MonthlyScheduleItem>> _loadCancelledItems({
    required String loadedRole,
    required Set<String> batches,
  }) async {
    final result = <_MonthlyScheduleItem>[];
  ...
  DateTime? _trainingDateForThisMonth(Map<String, dynamic> data) {
    final dateValue = data['date'];

    if (dateValue != null && _text(dateValue).isNotEmpty) {
      final parsed = _parseDate(dateValue);
      if (parsed != null && _isCurrentMonth(parsed)) {
        return parsed;
      }
      return null;
    }

    final day = _text(data['day']);
        ...
        return ValueListenableBuilder<String>(
          ...
          body: SafeArea(
            child: loading
                ...
                : RefreshIndicator(
                    onRefresh: _loadMonthlySchedule,
  ...
  Widget _summaryRow(bool isDark) {
    final trainingCount =
        monthlyItems.where((item) => item.category == "Training").length;
    final matchCount =
        monthlyItems.where((item) => item.category == "Match").length;
  ...
  Widget _scheduleTile({
    required bool isDark,
    required _MonthlyScheduleItem item,
  }) {
    return Container(
